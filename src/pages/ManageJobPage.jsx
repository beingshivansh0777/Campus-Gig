...
import { ArrowLeft, Pencil, Trash2, UserCircle2 } from "lucide-react";
...
function ManageJobPage() {
  ...
  const shortlistProposal = useShortlistProposal(jobId);
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {job.jobStatus === "CLOSED" ? (
            <button
              disabled
              title="Closed projects can't be edited"
              className="flex items-center gap-1.5 text-sm font-body font-medium text-faint border border-border px-3 py-2 rounded-lg opacity-50 cursor-not-allowed"
            >
              <Pencil size={14} /> Edit
            </button>
          ) : (
      ...
      {applicantsLoading ? (
        ...
        <div className="bg-surface border border-border rounded-xl p-8 text-center">
          <p className="font-body text-sm text-muted">
            No proposals in this status yet.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs font-body text-muted">
                    ...
                    <span>
                      Delivery:{" "}
                      {new Date(applicant.deliveryDate).toLocaleDateString()}
                    </span>
                  </div>
                </div>
