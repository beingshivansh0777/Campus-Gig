...
import RoleRestrictedModal from "../ui/RoleRestrictedModal";
...
function Footer() {
  ...
  const { token, isGig } = useAuthStore();
  const [restrictedMessage, setRestrictedMessage] = useState(null);

  const handleClientOnlyClick = (e) => {
    if (token && isGig) {
      e.preventDefault();
      setRestrictedMessage(
        "This feature is only available for Client accounts.",
      );
    }
  };

  const handleGigOnlyClick = (e) => {
    if (token && !isGig) {
      e.preventDefault();
      setRestrictedMessage(
        "This feature is only available for Gigger accounts.",
      );
    }
  };
